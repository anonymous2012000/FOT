...
// generates a random 128-bit number-- mainly for testing encode-evaluate-decode
...
mpz_class Decode_mode(const mpz_class& encoded_value, const std::vector<mpz_class>& random_values, int gamma, const mpz_class& modulus) {
  ...
  for (const auto& r : random_values) {
    ...
    if (additive_inverse < 0) {
      cout<<"\n in decode--additive_inverse < 0"<<endl;
...
int main() {
  ...
  int p_size = 30;// it is t in t-out-of-n OT
  ...
  for(int i = 0; i < number_of_tests; i++){
    ...
    for(int j = 0; j<number_of_OT; j++){
      for (int i = 0; i < n; ++i) {
        messages[j].push_back(generate_random_bigint(rng, message_size));
      }
    }

    for (int i = 0; i < n; ++i) {
      m.push_back(generate_random_bigint(rng, message_size));
    }

    if(p_size < 4){
      for (int i=0; i<number_of_OT; i++){
      m[collection_of_p[i][0]] = m[collection_of_p[i][1]];
    }
  }
  else{
    for (int i=0; i<number_of_OT; i++){
      m[collection_of_p[i][0]] = m[collection_of_p[i][1]];
      m[collection_of_p[i][2]]= m[collection_of_p[i][1]];
      m[collection_of_p[i][3]]= m[collection_of_p[i][4]];
    }
  }
  ...
  // GenRes
  vector<vector<mpz_class>> res_s = GenRes(m, number_of_OT, r, w, gamma, modulus);
  ...
  cout<<"\nin main-- Number of OT: "<<number_of_OT<<endl;
  for(int k = 0; k < number_of_OT; k++){
